...
def crear_usuario(nuevo_usuario):
    response = requests.post(BASE_URL, json=nuevo_usuario, auth=HTTPBasicAuth('admin', 'password123'))
...
def obtener_usuario_por_id(usuario_id):
    response = requests.get(f"{BASE_URL}/{usuario_id}", auth=HTTPBasicAuth('admin', 'password123'))
    if response.status_code == 200:
        usuario = response.json()
        print(f"Usuario encontrado: ID: {usuario['id']}, Nombre: {usuario['nombre']}")
    else:
        print("Error al obtener usuario:", response.status_code, response.text)

# Función para eliminar un usuario por ID
def eliminar_usuario(usuario_id):
    response = requests.delete(f"{BASE_URL}/{usuario_id}", auth=HTTPBasicAuth('admin', 'password123'))
    if response.status_code == 204:
        print("Usuario eliminado.")
    else:
        print("Error al eliminar usuario:", response.status_code, response.text)
...
if __name__ == '__main__':
    ...
    nuevo_usuario = {"id": 3, "nombre": "Dario"}
